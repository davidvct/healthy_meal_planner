...
def _parse_str_list(value: Any) -> tuple[str, ...]:
    parsed = parse_json(value, None)
    if isinstance(parsed, list):
        return tuple(dict.fromkeys(str(item).strip().lower() for item in parsed if str(item).strip()))
    if isinstance(value, str):
...
def normalize_recipe(record: dict[str, Any]) -> SolverRecipe:
    ...
    meal_types_raw = record.get("meal_types") or record.get("mealTypes")
    ...
    recipe_id = str(
        record.get("recipe_id") or record.get("recipeId") or record.get("id") or ""
    ).removeprefix("r")
    if not recipe_id:
    ...
    return SolverRecipe(
        ...
        sodium=parse_float(record.get("sodium"), 0.0),
        sugar=parse_float(record.get("sugar"), 0.0),
