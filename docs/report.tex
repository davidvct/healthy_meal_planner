...
\title{MealVitals: Market-to-Table Dietary Management}
\author{
\begin{tabular}[t]{c}
\bfseries Christopher Terence Ng, Heng Yan Si, Poh Jia Jun\\
e1112601@u.nus.edu, e0572838@u.nus.edu, e0010964@u.nus.edu\\[0.5em]
\bfseries Tan Xue Ying, Thien Vui Chia, Zheng Zhiqing\\
e0313593@u.nus.edu, e0818043@u.nus.edu, e0293907@u.nus.edu
\end{tabular}
}
...
\begin{abstract}
One in three Singapore residents live with chronic hyperlipidaemia or hypertension, a rate higher than OECD averages and one which remains stubbornly unchanged even as residents adopt healthier lifestyles post-COVID. We propose MealVitals, a cloud-native Software-as-a-Service application that enables users to track nutritional intake, generate personalised meal plans, and produce grocery lists -- supporting every step of the dietary management journey from market to table.
\end{abstract}
...
The 2024 National Population Health Survey~\cite{b1} conducted by the Ministry of Health (MOH) reports that approximately one in three Singapore residents have hyperlipidaemia or hypertension, despite a rise in the proportion of residents engaged in sufficient total physical activity from 75\% to 85\% since 2022. Coupled with rising obesity prevalence, these figures present an opportunity to improve dietary management through technology-enabled nutrition tracking.
...
\begin{enumerate}
    ...
    \item Helping residents make more informed choices through product labelling.
...
Consequently, residents frequently select food based on ``experience and knowledge'' rather than on evidence-based labelling~\cite{b3}. Even when residents can correctly identify the healthier option, the cognitive effort required to apply this knowledge throughout the food consumption pipeline -- from meal planning to grocery shopping -- presents a significant barrier to adherence. A clear opportunity therefore exists for a solution that simplifies dietary decision-making.

\subsection{Proposed Solution}

MealVitals addresses this gap by providing a cross-platform meal planning service that converts dietary constraints into personalised meal plans and grocery lists. The service targets two segments:
\begin{enumerate}
...
MealVitals generates personalised meal plans informed by health profiles, dietary constraints, and personal preferences, thereby improving adherence by simplifying meal planning and grocery purchasing. Studies~\cite{b4} have shown that elderly residents with chronic conditions often struggle with digital tools; MealVitals addresses this barrier by enabling a digitally proficient caregiver to plan meals on their behalf. Importantly, MealVitals provides dietary support rather than medical advice, which remains the responsibility of healthcare professionals.
...
MealVitals adopts a freemium model: all users receive access to core features for meal management and health monitoring at no cost, while advanced personalisation capabilities -- such as automated meal plan generation and adaptive recommendations -- are reserved for premium subscribers. Table~\ref{tab:tiers} summarises the feature set for each tier.
...
Meal Actions (swap, remove, favourite) & \checkmark & \checkmark \\
...
Meal Selection & Pre-filtered by profile & Personalised recommendations \\
...
MealVitals & \$9.9/mo & No & Yes & Yes & Yes \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{center}
...
MealVitals differentiates itself through two capabilities absent from competing products: constraint-based meal planning -- accounting for dietary restrictions, nutritional targets, user preferences, and recent consumption history -- and automated grocery list generation. Together, these features support users throughout the entire dietary management journey, from market to table.

MealVitals does not currently offer dish recognition; however, integration with the FoodAI API~\cite{b5}, the same service consumed by Healthy~365, would enable rapid feature parity with competitors.
...
\textit{On-Premises Deployment:} Both Capital Expenditure (CAPEX) and Operating Expenditure (OPEX) are considered. CAPEX covers two server racks deployed in Singapore, depreciated over three years; Year~1 must be provisioned for the peak capacity projected in Year~3. OPEX includes electricity, broadband, and facility overheads.

\textit{Cloud Deployment:} As noted by Lenovo~\cite{b10}, cloud deployment incurs only OPEX through pay-as-you-go subscription fees. No upfront CAPEX is required.
...
Cloud TCO is 28\% lower than on-premises over three years (Table~\ref{tab:tco}), driven primarily by the elimination of upfront capital expenditure.
...
Monthly cloud cost scales with user growth, whereas an on-premises deployment must be provisioned for Year~3 peak capacity from the outset. Although the cloud's monthly cost eventually exceeds the on-premises equivalent in Year~3, the cumulative three-year savings remain substantial.

%-----------------------------------------------------------------------
\section{Architecture \& Implementation}

\subsection{System Architecture \& Data Modelling}
...
\begin{itemize}
    ...
    \item \textbf{Application layer} -- a FastAPI service exposing RESTful endpoints for authentication and business logic, running on Uvicorn.
    \item \textbf{Database layer} -- PostgreSQL with raw SQL via psycopg for data access.
\end{itemize}

\subsubsection{System Design}
The application is deployed on Google Cloud Platform (GCP), as illustrated in Fig.~\ref{fig:arch}:
\begin{itemize}
    \item \textbf{Cloud DNS} -- resolves the hostname \texttt{mealvitals.app} to the Cloud Load Balancer's public IPv4 address.
    \item \textbf{Cloud Load Balancer} -- performs TLS termination and path-based routing, directing static asset requests to Cloud Storage and API requests to Cloud Run.
    ...
    \item \textbf{Cloud SQL} -- stores all application and authentication data; deployed within a VPC to avoid public IP exposure.
\end{itemize}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{architecture.pdf}}
\caption{System architecture deployed on Google Cloud Platform.}
\label{fig:arch}
\end{figure}
...
The meal recommender is built on Google OR-Tools CP-SAT~\cite{b7}, a constraint programming solver that serves as the optimisation engine. Premium users receive personalised meal recommendations accounting for dietary preferences, allergies, and health conditions. The recommender generates plans for breakfast, lunch, and dinner across horizons of one to seven days. Users may preserve previously accepted meals as fixed assignments and request recommendations only for remaining empty slots, enabling an interactive planning workflow. This formulation constitutes a constraint optimisation problem~\cite{b6}; CP-SAT is well suited because it natively handles binary and integer decision variables under both hard constraints and soft objective trade-offs.

\begin{table}[htbp]
\caption{Optimisation Components}
\label{tab:optim}
\begin{center}
...
Pre-processing & Recipes violating allergy, diet, or medical suitability rules are filtered prior to optimisation \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
The primary objective is to minimise deviation from the user's target intake, with emphasis on calories, protein, and carbohydrates. Secondary penalties discourage exceeding fat, sodium, and sugar targets; repeating recipes excessively; selecting cautionary dishes; and reusing recently served meals.
...
CP-SAT is preferable to simpler scoring-based approaches because it performs global optimisation over the entire plan, simultaneously satisfying nutritional targets, dietary restrictions, preserved assignments, partial-week generation, dish-count limits, and variety constraints within a unified framework.

\subsection{System Interaction and Behavioural Design}

MealVitals supports four primary use cases: (1)~managing health profiles, (2)~generating meal plans, (3)~retrieving shopping lists, and (4)~tracking health metrics. Fig.~\ref{fig:seq} (Appendix~\ref{app:seq}) presents the detailed interaction sequence for the meal plan autofill flow, which exercises the full stack from frontend through the CP-SAT solver.

The full sequence diagram is provided in Appendix~\ref{app:seq}.

\subsection{Security, Authentication and Secret Management}

MealVitals handles sensitive user data including email addresses, dietary preferences, allergies, and health-related profiles. As a multi-tenant SaaS platform, the system must ensure that only authenticated users can access the application and that each user's data is securely isolated. The security model follows a five-stage process (Table~\ref{tab:security}).

\begin{table*}[htbp]
\caption{Security Model -- Five-Stage Process}
\label{tab:security}
\begin{center}
...
Upon successful login, the backend issues a signed JWT containing user identity (\texttt{sub}) and expiry (\texttt{exp}) claims; signing keys are stored in GitHub Secrets and injected during CI/CD &
...
Guarantees that users maintain a valid session before accessing resources and prevents unauthorised cross-origin API requests \\
\addlinespace
4. Token Validation &
...
The growth of MealVitals is evaluated across three funnels: acquisition, retention, and monetisation~\cite{b8}. Acquisition employs leading indicators to measure onboarding velocity. Retention uses engagement and guardrail metrics to monitor sustained usage. Monetisation relies on lagging indicators to assess the health outcomes delivered to users. All metrics are collected through Google Analytics.
...
\begin{itemize}
    \item \textit{Improved Cardiometabolic Health} -- proportion of active users who report reduced cholesterol, lower blood pressure, or improved glycaemic control after six months. Data is collected via a biannual survey distributed automatically through Google Cloud Scheduler.
\end{itemize}
...
\textbf{Data Coverage.} The current meal dataset is not contextualised to Singapore's culinary culture. Future work could incorporate local dishes by enabling community-contributed recipes, with AI used to estimate nutritional values from user-submitted images or text descriptions.
...
\textbf{Solver Parallelisation.} The meal recommender currently generates recommendations sequentially across days. Parallelising the solver across independent day-partitions would improve generation latency and balance computational load under increasing user demand.
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{sequence.pdf}
\caption{Sequence diagram for the meal plan autofill flow.}
\label{fig:seq}
\end{figure*}
